...
create table bmsql_warehouse (
  ...
  w_state     char(2),
  w_zip       char(9)
) partition by hash(w_id) partitions 64;
...
create table bmsql_district (
  ...
  d_state      char(2),
  d_zip        char(9)
) partition by hash(d_w_id,d_id) partitions 16;
...
) partition by hash(no_w_id) partitions 64;
...
create table bmsql_oorder (
  ...
  o_all_local  integer,
  o_entry_d    timestamp
) partition by hash(o_w_id) partitions 64;
...
) partition by hash(ol_w_id) partitions 64;
...
create table bmsql_stock (
  ...
  s_dist_09    char(24),
  s_dist_10    char(24)
) partition by hash(s_w_id,s_i_id) partitions 64;
