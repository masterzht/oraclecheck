...
REM
REM $Header: 1366133.1 sqlhcxec.sql 12.1.12 2015/09/11 carlos.sierra mauro.pagano abel.macias@oracle.com $
REM
...
REM
REM SCRIPT
REM   sqlhcxec.sql SQL Health-Check (execute mode)
REM
REM DESCRIPTION
...
REM
REM PRE-REQUISITES
REM   1. Execute as the application user that can execute the SQL
REM      passed in the input script. This user must have access to
REM      data dictionary views.
REM   2. The SQL for which this script is executed must be provided
REM      in a stand-alone script containing the SQL Text with its
REM      binds if appropriate.
REM
REM PARAMETERS
REM   1. Oracle Pack license (Tuning or Diagnostics or None) T|D|N
REM   2. SCRIPT name which contains SQL and its binds (required)
REM
REM EXECUTION
REM   1. Start SQL*Plus connecting as the application user that can
REM      execute the SQL passed in the input script. This user must
REM      have access to data dictionary views.
REM   2. Execute script sqlhcxec.sql passing values for parameters.
REM
REM EXAMPLE
...
REM
REM NOTES
REM   1. For possible errors see sqlhcxec.log.
...
COL prev_sql_id NEW_V prev_sql_id FOR A13;
-- dummy execution below, just to create cursor now and not when actually executed to get sql_id of sql on user script
SELECT prev_sql_id FROM v$session WHERE sid = (SELECT sid FROM v$mystat WHERE ROWNUM = 1) AND status = 'ACTIVE' AND ROWNUM = 1;
...
PRO
DEF file_with_sql = '^2';
DEF input_parameter = '^^file_with_sql.';
PRO
PRO Values passed:
PRO License: "^^input_license."
PRO Script : "^^file_with_sql."
PRO
PRO In case of a disconnect review sqlhcxec_^^unique_id._error.log
PRO

SET TERM OFF ECHO ON HEA ON LIN 2000 PAGES 1000 TRIMS ON TI OFF TIMI OFF SERVEROUT OFF;
WHENEVER SQLERROR EXIT SQL.SQLCODE;

SPO sqlhcxec_^^unique_id._error.log;

ALTER SESSION SET MAX_DUMP_FILE_SIZE = '1G';
ALTER SESSION SET TRACEFILE_IDENTIFIER = "10046_10053_^^unique_id.";
ALTER SESSION SET EVENTS '10046 TRACE NAME CONTEXT FOREVER, LEVEL 12';
ALTER SESSION SET EVENTS '10053 TRACE NAME CONTEXT FOREVER';

-- track v$mystat before running the SQL
INSERT /* myself */ INTO plan_table (statement_id, parent_id, options, time, cpu_cost)
SELECT 'B^^unique_id.', a.statistic#, b.name, b.class, a.value
...
-- gets sql_id of most recent sql, hopefully it would be the one from user script
SELECT prev_sql_id FROM v$session WHERE sid = (SELECT sid FROM v$mystat WHERE ROWNUM = 1) AND status = 'ACTIVE' AND ROWNUM = 1;

-- track v$mystat after running the SQL (and query V$SESSION to find the right SQL_ID)
INSERT /* myself */ INTO plan_table (statement_id, parent_id, options, time, cpu_cost)
SELECT 'A^^unique_id.', a.statistic#, b.name, b.class, a.value
  FROM v$mystat a, v$statname b
 WHERE a.statistic# = b.statistic#
   AND a.value <> 0;
...
SET TERM ON;
PRO Ignore MOVE or MV error below
SET TERM OFF;

-- output of script is not copied into zip file so temp error filename gets renamed and left behind
HOS move sqlhcxec_^^unique_id._error.log sqlhcxec_^^unique_id._output_from_user_script.txt
HOS mv sqlhcxec_^^unique_id._error.log sqlhcxec_^^unique_id._output_from_user_script.txt

COL sql_id NEW_V sql_id FOR A13;
SELECT /* myself */ sql_id FROM v$sql WHERE sql_text LIKE '%^^unique_id.%' AND sql_text NOT LIKE '%/* myself */%' AND ROWNUM = 1;
SELECT SUBSTR(sql_text, 1, 100) sql_text_from_unique_id FROM v$sql WHERE sql_id = '^^sql_id.' AND ROWNUM = 1;
SELECT SUBSTR(sql_text, 1, 100) sql_text_from_prev_sql_id FROM v$sql WHERE sql_id = '^^prev_sql_id.' AND ROWNUM = 1;
-- if sql had unique_id on it then take sql_id from it else from prev sql as per v$session
SELECT NVL('^^sql_id.', '^^prev_sql_id') sql_id FROM dual;
...
BEGIN
  IF '^^sql_id.' IS NULL THEN
    RAISE_APPLICATION_ERROR(-20200, 'SQL_ID "^^sql_id." not found in memory.');
  END IF;
END;
/
...
PRO <li><a href="#sess_stats">Session Statistics (V$MYSTAT)</a></li>
...
*
*--------------------------- */
PRO <a name="sess_stats"></a><h2>Session Statistics (V$MYSTAT)</h2>
PRO
PRO Session statistics for child cursors of ^^sql_id. captured right before and after the execution.
PRO
PRO <table>
PRO
PRO <tr>
...
SELECT CHR(10)||'<tr>'||CHR(10)||
    -- omit class for now   '<td>'||DECODE(after.class,1,'User',2,'Redo',4,'Enqueue',8,'Cache',16,'OS',32,'Real Application Clusters',64,'SQL',128,'Debug')||'</td>'||CHR(10)||
       '<td>'||after.name||'</td>'||CHR(10)|| 
	   '<td class="r">'||TO_CHAR(after.value - nvl(before.value,0),'999999999999')||'</td>'||CHR(10)||   
	   '</tr>'
  FROM (SELECT parent_id statistic#, cpu_cost value 
          FROM plan_table 
		 WHERE statement_id = 'B^^unique_id.') before,
       (SELECT parent_id statistic#, options name, time class, cpu_cost value 
	      FROM plan_table 
		 WHERE statement_id = 'A^^unique_id.') after
 WHERE before.statistic#(+) = after.statistic#
   AND before.value(+) <> after.value
 ORDER BY --DECODE(after.class,1,'User',2,'Redo',4,'Enqueue',8,'Cache',16,'OS',32,'Real Application Clusters',64,'SQL',128,'Debug'),
       after.name;	   
...
-- 150826 use diag dir if available.
select value||DECODE(INSTR(value, '/'), 0, '\', '/') udump_path from sys.v_$diag_info where name = 'Diag Trace';
...
-- tkprof for trace from execution of script
SET TERM ON;
PRO ### tkprof commands below may error out with "could not open trace file". disregard error.
SET TERM OFF;
HOS tkprof ^^udump_path.*_10046_10053_^^unique_id.*.trc ^^files_prefix._7_tkprof_from_user_script_exec.txt

-- windows workaround (copy below will error out on linux and unix)
HOS copy ^^udump_path.*_10046_10053_^^unique_id.*.trc ^^udump_path.^^unique_id._10046_10053.trc
SET TERM ON;
PRO ### tkprof commands below may error out with "could not open trace file". disregard error.
SET TERM OFF;
HOS tkprof ^^udump_path.^^unique_id._10046_10053.trc ^^files_prefix._7_tkprof_from_user_script_exec.txt

SET TERM ON;
PRO Ignore CP or COPY error below
SET TERM OFF;

-- copy 10046/10053 trace
SET TERM ON;
PRO Ignore CP or COPY error below
SET TERM OFF;
HOS cp ^^udump_path.*_10046_10053_^^unique_id.*.trc ^^files_prefix._6_10046_10053_trace_from_user_script_exec.trc
HOS copy ^^udump_path.*_10046_10053_^^unique_id.*.trc ^^files_prefix._6_10046_10053_trace_from_user_script_exec.trc
...
HOS zip -m ^^files_prefix..zip ^^files_prefix._6_10046_10053_trace_from_user_script_exec.trc
HOS zip -m ^^files_prefix..zip ^^files_prefix._7_tkprof_from_user_script_exec.txt
--HOS zip -m ^^files_prefix..zip sqlhcxec_^^unique_id._output_from_user_script.txt
...
HOS unzip -l &&files_prefix..zip
CL COL;
UNDEF 1 2 method script mos_doc doc_ver doc_date doc_link bug_link input_parameter file_with_sql input_license sid unique_id prev_sql_id sql_id signature signaturef license udump_path sqldx_prefix sqldx_output;
