...
REM $Header: 215187.1 sqlt_insert_hgrm_bucket.sql 11.4.5.0 2012/11/21 carlos.sierra $
REM
...
REM
REM SCRIPT
REM   sqlt/utl/xgram/sqlt_insert_hgrm_bucket.sql
REM
REM DESCRIPTION
REM   Inserts ONE bucket to an existing column's CBO histogram
REM   while preserving remaining buckets and column statistics.
...
COL p_value NEW_VALUE p_value FOR A100;
COL p_size NEW_VALUE p_size FOR A100;
...
PRO
ACC p_value PROMPT 'Value &&local_data_format.[UNKNOWN]: ';
PRO
ACC p_size PROMPT 'Size [0]: ';
PRO
...
PRO
SET TERM OFF;
SELECT TRIM('&&p_value.') p_value FROM DUAL;
SELECT NVL(TRIM('&&p_size.'), '0') p_size FROM DUAL;
...
SPO sqlt_insert_hgrm_bucket_&&log_time..log
SET TERM ON;
PRO sqlt_insert_hgrm_bucket_&&log_time..log
PRO
...
PRO p_force        : '&&p_force.'
PRO
PRO EXEC &&tool_administer_schema..sqlt$s.insert_hgrm_bucket(p_ownname => '&&p_ownname.', p_tabname => '&&p_tabname.', p_colname => '&&p_colname.', p_value => '&&p_value.', p_size => &&p_size., p_partname => '&&p_partname.', p_preserve_size => &&p_preserve_size., p_no_invalidate => &&p_no_invalidate., p_force => &&p_force.);;
PRO
SET SERVEROUT ON SIZE 1000000;
BEGIN
  &&tool_administer_schema..sqlt$s.insert_hgrm_bucket (
    p_ownname       => '&&p_ownname.',
    p_tabname       => '&&p_tabname.',
    p_colname       => '&&p_colname.',
    p_value         => '&&p_value.',
...
PRO
PRO SQLT_INSERT_HGRM_BUCKET completed.
SPO OFF;
