...
case when ksppinm = '_optim_peek_user_binds' and ksppstvl <> 'TRUE'
      then 'Recommend set to default value <TRUE>, Peep through statistical information histogram
 binding variables'
     ...
     when ksppinm = '_optimizer_adaptive_cursor_sharing' and ksppstvl <> 'TRUE'
      then 'Recommend set to <TRUE> to solves the execution plan remains unchanged after binding
variable peeping issue'
     when ksppinm = '_optimzer_adaptive_extended_cursor_sharing' and ksppstvl <> 'UDO'
      then 'should be set to <UDO> to solves the problem that the execution plan remains unchange
d after binding variable peeping issue'
     when ksppinm = '_optimizer_extended_cursor_sharing_rel' and  ksppstvl <> 'SIMPLE'
      then 'Should be set to <SIMPLE> to solves the problem that the execution plan remains uncha
nged after binding variable peeping issue'
     ...
     when ksppinm = '_serial_direct_read' and ksppstvl <> 'auto'
      then 'Recommend to set to <auto>, Only when the table is small can we consider setting it t
o never'
     ...
     when ksppinm = '_use_adaptive_log_file_sync' and ksppstvl ='TRUE'
      then '11203 and above Version default <TRUE> May Cause log sync Issue,refer to DOC 1462942.
1'
     ...
     when ksppinm = '_clusterwide_global_transactions' and ksppstvl <> 'TRUE'
      then 'In RAC Env,May Cause 2PC Transaction Issue'
...
name,
value,
case when name = 'open_cursors' and (value < 300 or value > 1000) then 'open_cursors recommend to
 set to more than <300>, 1000 is enough'
     when name = 'control_file_record_keep_time' and value <= 7 then 'control_file_record_keep_ti
me recommend to set to <15>, 30 is enough,Doc 47322.1'
     when name = 'cursor_sharing' and value <> 'EXACT' then 'Recommend to set to <EXACT>, <FORCE>
 Forcing SQL that does not use binding variables into binding variables'
     when name = 'audit_trail' and value <> 'NONE' then 'If necessary set value to <NONE> to clos
e Audit'
     when name = 'sesssion_cached_cursors' and value > 100 then 'No need to set too large, Set th
e maximum number of closed cursors that can be cached in each session.'
     when name = 'deferred_segment_creation' and value <> 'FALSE' then '<TRUE> will result in the
 failure to export an empty table ,Mos 1216282.1'
     when name = 'enable_ddl_logging' and value <> 'TRUE' then '<TRUE> enable alert log records D
DL statements'
     when name = 'undo_retention' and value < 1800 then 'Recommend to set to 10800 and set a larg
e enough undo tablespace'
     when name = 'statistics_level' and value <> 'TYPICAL' then 'Recommend to set it to the defau
lt value <TYPICAL>'
...
>'
     when name = 'sga_target' and value = 0 then 'Recommend to set the same value with sga_max_si
ze'
     when name = 'sga_max_size' and value < 100 then 'According to the business and system memory
 settings'
     ...
     when name = 'fast_start_parallel_rollback' and value = 'FALSE'
      then '<FALSE> means Parallel rollback is disabled, <LOW> Limits the maximum degree of paral
lelism to 2 * CPU_COUNT'
end as adivce
from v$parameter
where name in (
'open_cursors',
