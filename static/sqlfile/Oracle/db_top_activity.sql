...
ROUND(100.0 * sum(case when WAIT_CLASS = 'ON CPU' then 1 else 0 end)/count(*), 2) as "ON_CPU",
...
ROUND(100.0 * sum(case when WAIT_CLASS = 'User I/O' then 1 else 0 end)/count(*), 2) as "User_IO",
ROUND(100.0 * sum(case when WAIT_CLASS = 'System I/O' then 1 else 0 end)/count(*), 2) as "System_IO",
